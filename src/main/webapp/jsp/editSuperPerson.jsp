...
<html>
    <head>
        <title>SuperPerson Center - Edit SuperPerson</title>
        <!-- Bootstrap core CSS -->
                ...
                <sf:form class="form-horizontal" role="form" modelAttribute="superperson" 
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-md-10">
                                <select name="orgSelection" multiple>
                                    <c:if test="${empty orgsSelected}">
                                        <option value="none" selected hidden/>
                                    </c:if>
                                    <option value="none">None</option>
                                    <c:forEach var="orgsSelected" items="${orgsSelected}">
                                        <option selected="selected" value="${orgsSelected.organizationId}">
                                            <c:out value="${orgsSelected.name}"/>
                                        </option>
                                    </c:forEach>
                                    <c:forEach var="orgsNotSelected" items="${orgsNotSelected}">
                                        <option value="${orgsNotSelected.organizationId}">
                                            <c:out value="${orgsNotSelected.name}"/>
                    ...
                    <div class="form-group">
                        <div class="col-md-offset-2 col-md-8">
                            <input type="submit" class="btn btn-default" value="Edit SuperPerson"/>
                        </div>
                    </div>
                </sf:form>
